...
function build_conf()
{
	...
	for ((i=0; i < ${#ioservices[*]}; i++, M0D++)); do
	    ...
	    if [ $ENABLE_CAS -eq 1 ] ; then
	        ...
	        NAMES_NR=8
                if [ $ENABLE_DTM -eq 1 ] ; then
                    local DTM0_NAME="$DTM0_FID_CON:$i"
	            local DTM0_OBJ="{0x73| (($DTM0_NAME), @M0_CST_DTM0, [1: $iosep], [1: \"origin:in-persistent\"], [0])}"
	            NAMES_NR=9
                fi
	    fi
	    ...
	    IOS_NAMES[$i]="$IOS_NAME, $ADDB_NAME, $SNS_REP_NAME, $SNS_REB_NAME, \
	                   $RM_NAME${CAS_NAME:+,} $CAS_NAME, $DIX_REP_NAME, \
                           $DIX_REB_NAME${DTM0_NAME:+,} $DTM0_NAME"
	...
	if [ $ENABLE_CAS -eq 1 ] ; then
        ...
        # if CAS and DTM0 is enabled then need to add the section related
        # to client DTM0 configuration, example from all2all test (config
        # is generated by Hare):
        #
        # {0x72| ((^r|1:5), [8: 1, 1, 1, 1, 1, 1, 1, 1], 0x8000000, 0xf6f800, 0xf6f800, 0xf6f800, "inet:tcp:10.230.249.99@21501", [2: ^s|1:31, ^s|1:32])},
        # {0x73| ((^s|1:31), @M0_CST_RMS, [1: "inet:tcp:10.230.249.99@21501"], [0], [0])},
        # {0x73| ((^s|1:32), @M0_CST_DTM0, [1: "inet:tcp:10.230.249.99@21501"], [1: "origin:in-volatile"], [0])},
        #
        # and consider ^r|1:5 in node resources list, example
        # {0x6e| ((^n|1:0), 0x3dbe, 8, 0, 0, [6: ^r|1:0, ^r|1:1, ^r|1:2, ^r|1:3, ^r|1:4, ^r|1:5])},
